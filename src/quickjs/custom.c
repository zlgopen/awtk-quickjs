...
JSValue quickjs_create_pointer(JSContext* ctx, const void* ptr, const char* type) {
...
static async_callback_info_t* async_callback_info_create(JSContext* ctx, JSValue func) {
...
JSValue wrap_widget_on(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  ret_t ret = RET_FAIL;

  if (argc >= 3) {
    widget_t* widget = (widget_t*)quickjs_get_pointer(ctx, argv[0], "widget_t*");
    uint32_t type = (uint32_t)quickjs_get_int_value(ctx, argv[1]);
    async_callback_info_t* info = async_callback_info_create(ctx, JS_DupValue(ctx, argv[2]));
...
JSValue wrap_emitter_on(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  ret_t ret = RET_OK;

  return quickjs_create_int(ctx, ret);
}

JSValue wrap_locale_info_on(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  ret_t ret = RET_OK;
...
JSValue wrap_timer_add(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  uint32_t ret = TK_INVALID_ID;

  if (argc >= 2) {
    async_callback_info_t* info = async_callback_info_create(ctx, JS_DupValue(ctx, argv[0]));
...
JSValue wrap_idle_add(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  uint32_t ret = TK_INVALID_ID;
...
JSValue wrap_object_foreach_prop(JSContext* ctx, JSValueConst this_val, int argc,
                                 JSValueConst* argv) {
  ret_t ret = RET_OK;
...
JSValue wrap_widget_foreach(JSContext* ctx, JSValueConst this_val, int argc, JSValueConst* argv) {
  ret_t ret = RET_FAIL;
